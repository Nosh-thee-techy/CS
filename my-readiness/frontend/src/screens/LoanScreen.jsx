...
function ApplyFlow({ profile, hasDebt }) {
  ...
  const eligible = Boolean(profile.disbursementEligible) && !hasDebt;
...
function ApplyWizard({ profile, eligible, hasDebt }) {
  const { t } = useTranslation();
  const { applyForLoan, sendLoanOtp, loanBusy, loanError, otpHint } = useReadiness();
      ...
      {step === 3 && (
        <form
          ...
          onSubmit={(event) => {
            ...
            applyForLoan(purpose, otpCode, max);
          }}
        >
          ...
          <div className="rounded-[24px] border border-white/15 bg-panel p-4 space-y-3">
            <p className="text-sm leading-relaxed text-mute">{t("loan.otpHelp")}</p>
            <button
              type="button"
              onClick={onSendCode}
              disabled={otpBusy}
              className="rounded-full border border-white/20 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40"
            >
              {otpBusy ? t("loan.otpSending") : otpSent ? t("loan.otpResend") : t("loan.otpSend")}
            </button>
            {otpSent && (
              <label className="block">
                <span className="mb-2 block text-sm font-medium text-mute">{t("loan.otpCode")}</span>
                <input
                  value={otpCode}
                  ...
                  maxLength={6}
                  placeholder="000000"
                  className="h-14 w-full rounded-full border border-white/20 bg-white/5 px-5 text-center text-xl tracking-[0.4em] text-white outline-none focus:border-ember"
                />
              </label>
            )}
            {otpHint && <p className="text-xs text-mute">{t("loan.otpDemo", { code: otpHint })}</p>}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="min-w-0 flex-1">
              <PillButton type="submit" variant="ember" disabled={loanBusy || !otpSent || otpCode.length < 6}>
